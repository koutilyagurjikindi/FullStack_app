...
router.param("article", function(req, res, next, slug) {
  console.log(slug,"kdfjdkjfkdjfkdjkf")
...
router.post("/:article/comments", auth.required, function(req, res, next) {
  User.findById(req.payload.id)
    .then(function(user) {
      ...
      (comment.article = req.article), (comment.author = user);

      var articles = req.article;
      return comment.save().then(function() {
        articles.comments = articles.comments.concat([comment]);
    ...
    .catch(error => {
      console.log(error, "dlfkdlkfldkfld");
    });
});

router.get("/:article/comments", auth.optional, function(req, res, next) {
  Promise.resolve(req.payload ? User.findById(req.payload.id) : null).then(
    function(user) {
      return req.article
        .populate({
          path: "comments",
...
router.get('/feeds', auth.required, function(req, res, next) {
  var limit = 20;
  var offset = 0;
